...
@interface ZANetworkTests : XCTestCase
@property (nonatomic, strong) NSURL *url;
@property (nonatomic, strong) ZANetwork *network;
@end

@implementation ZANetworkTests

- (void)setUp {
    // Put setup code here. This method is called before the invocation of each test method in the class.
    _url = [NSURL URLWithString:@"http://172.16.90.61:58080/a?service=zall&project=dddssss"];
    _network = [[ZANetwork alloc] init];
    [ZallDataSDK startWithConfigOptions:[[ZAConfigOptions alloc] initWithServerURL:_url.absoluteString launchOptions:nil]];
}
...
- (void)testFixNullProjectWithH5UserAgentError {
    ...
    ZANetwork *network = [[ZANetwork alloc] init];
    NSString *appendingAgent = [NSString stringWithFormat: @" /za-sdk-ios/zall-verify/%@?%@ ", network.host, network.project];
    NSString *agentExpectation = [NSString stringWithFormat: @" /za-sdk-ios/zall-verify/%@?dddssss ", network.host];
    XCTAssertTrue([appendingAgent isEqualToString:agentExpectation]);
}

#pragma mark - URL Method
- (void)testGetHostWithURL {
    NSString *host = [ZAURLUtils hostWithURL:_url];
    XCTAssertEqualObjects(host, @"172.16.90.61");
}

- (void)testGetHostWithNilURL {
    NSString *host = [ZAURLUtils hostWithURL:nil];
    XCTAssertNil(host);
}

- (void)testGetHostWithURLString {
    NSString *host = [ZAURLUtils hostWithURLString:@"https://www.google.com"];
    XCTAssertEqualObjects(host, @"www.google.com");
}

- (void)testGetHostWithMalformedURLString {
    NSString *host = [ZAURLUtils hostWithURLString:@"google.com"];
    XCTAssertNil(host);
}

- (void)testGetQueryItemsWithURL {
    NSDictionary *items = [ZAURLUtils queryItemsWithURL:_url];
    BOOL isEqual = [items isEqualToDictionary:@{@"project": @"dddssss", @"service": @"zall"}];
    XCTAssertTrue(isEqual);
}

- (void)testGetQueryItemsWithNilURL {
    NSDictionary *items = [ZAURLUtils queryItemsWithURL:nil];
    XCTAssertNil(items);
}

- (void)testGetQueryItemsWithURLString {
    NSDictionary *items = [ZAURLUtils queryItemsWithURLString:@"https://sdk-test.datasink.zalldata.cn/sa?project=zhangminchao&token=95c73ae661f85aa0"];
    BOOL isEqual = [items isEqualToDictionary:@{@"project": @"zhangminchao", @"token": @"95c73ae661f85aa0"}];
    XCTAssertTrue(isEqual);
}

- (void)testGetQueryItemsWithNilURLString {
    NSDictionary *items = [ZAURLUtils queryItemsWithURLString:nil];
    XCTAssertNil(items);
}

#pragma mark - Server URL
- (void)testDebugOffServerURL {
    XCTAssertTrue([self.network.serverURL.absoluteString isEqualToString:self.url.absoluteString]);
   
}

- (void)testDebugOnlyServerURL {
    self.network.debugMode = ZallDataDebugOnly;
    XCTAssertTrue([self.network.serverURL.lastPathComponent isEqualToString:@"a"]);
}

- (void)testDebugAndTrackServerURL {
    self.network.debugMode = ZallDataDebugAndTrack;
    XCTAssertTrue([self.network.serverURL.lastPathComponent isEqualToString:@"a"]);
}

- (void)testGetServerURLHost {
    XCTAssertTrue([self.network.host isEqualToString:@"172.16.90.61"]);
}

- (void)testGetServerURLDefaultHost {
    NSURL *url = [NSURL URLWithString:@"http://172.16.90.61"];
    ZANetwork *network = [[ZANetwork alloc] init];
    XCTAssertTrue([network.host isEqualToString:@"172.16.90.61"]);
}

- (void)testGetServerURLProject {
    XCTAssertTrue([self.network.project isEqualToString:@"dddssss"]);
}

- (void)testGetServerURLDefaultProject {
    NSURL *url = [NSURL URLWithString:@"https://sdk-test.datasink.zalldata.cn/sa?token=95c73ae661f85aa0"];
    ZANetwork *network = [[ZANetwork alloc] init];
    XCTAssertTrue([network.project isEqualToString:@"dddssss"]);
}

- (void)testGetServerURLToken {
    XCTAssertTrue([self.network.token isEqualToString:@""]);
}

- (void)testGetServerURLDefaultToken {
    NSURL *url = [NSURL URLWithString:@"https://sdk-test.datasink.zalldata.cn/sa"];
    ZANetwork *network = [[ZANetwork alloc] init];
...
- (void)testCustomCertificate {
    ...
    ZASecurityPolicy *securityPolicy = [ZASecurityPolicy policyWithPinningMode:ZASSLPinningModeCertificate];
