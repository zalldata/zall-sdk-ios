//
// ZALinkHandlerTests.m
// ZallDataSDKTests
//
// Created by guo on 2020/1/16.
...
#import <ZallDataSDK/ZAConfigOptions.h>
#import "ZallDataSDK+ZADeeplink.h"

@interface ZALinkHandlerTests : XCTestCase

@property (nonatomic, strong) ZADeeplinkManager *linkHandler;

@end

@implementation ZALinkHandlerTests

- (void)setUp {
    NSString *urlString = @"https://www.zalldata.cn?utm_content=1&utm_campaign=1&utm_medium=1&utm_source=1&utm_term=1&channel=1&source=1&key=value";
    NSURL *url = [NSURL URLWithString:urlString];
    NSDictionary *launchOptions = @{UIApplicationLaunchOptionsURLKey: url};
    ZAConfigOptions *options = [ZAConfigOptions configWithServerURL:@"" launchOptions:launchOptions];
    options.enableSaveDeepLinkInfo = YES;
    options.sourceChannels = @[@"source", @"channel", @"device_id"];
    _linkHandler = [[ZADeeplinkManager alloc] init];
...
- (void)testURLValidity {
    NSURL *invalid1 = [NSURL URLWithString:@"https://www.zalldata.cn"];
    XCTAssertFalse([_linkHandler canHandleURL:invalid1]);

    NSURL *invalid2 = [NSURL URLWithString:@"https://www.zalldata.cn?utm_test=test&key=value"];
    XCTAssertFalse([_linkHandler canHandleURL:invalid2]);

    NSURL *valid1 = [NSURL URLWithString:@"https://www.zalldata.cn?source=source"];
    XCTAssertTrue([_linkHandler canHandleURL:valid1]);

    NSURL *valid2 = [NSURL URLWithString:@"https://www.zalldata.cn?utm_content=content"];
...
- (void)testNormalURL {
    NSURL *url = [NSURL URLWithString:@"https://www.zalldata.cn?utm_content=2&utm_campaign=2&channel=2&key=value"];
...
- (void)testEmptyContentOfPart {
    NSURL *url = [NSURL URLWithString:@"https://www.zalldata.cn?utm_content=&utm_campaign=3&channel=&key=value"];
    XCTAssertTrue([_linkHandler canHandleURL:url]);
...
- (void)testEmptyContentOfAll {
    NSURL *url = [NSURL URLWithString:@"https://www.zalldata.cn?utm_content=&utm_campaign=&channel=&key=value"];
    XCTAssertTrue([_linkHandler canHandleURL:url]);
...
- (void)testNoQueryURL {
    NSURL *url = [NSURL URLWithString:@"https://www.zalldata.cn"];
...
- (void)testFilterReservedProperty {
    NSURL *url = [NSURL URLWithString:@"https://www.zalldata.cn?utm_content=2&device_id=2"];
    XCTAssertTrue([_linkHandler canHandleURL:url]);

    [_linkHandler handleURL:url];
    NSDictionary *latest = [_linkHandler latestUtmProperties];
    XCTAssertTrue(latest.count == 2);
    XCTAssertTrue([latest[@"$latest_utm_content"] isEqualToString:@"2"]);

    NSDictionary *utm = [_linkHandler utmProperties];
    XCTAssertTrue(utm.count == 2);
    XCTAssertTrue([utm[@"$utm_content"] isEqualToString:@"2"]);
}
