...
static NSString *const kZAAppDeeplinkLaunchEvent = @"$AppDeeplinkLaunch";
static NSString *const kZADeeplinkMatchedResultEvent = @"$AppDeeplinkMatchedResult";
static NSString *const kZAEventPropertyDeepLinkURL = @"$deeplink_url";

static NSString *const kSavedDeepLinkInfoFileName = @"latest_utms";

@interface ZAEventDeepLinkLaunchObject : ZAEventPresetTrackObject

- (instancetype)init;

@end

@implementation ZAEventDeepLinkLaunchObject

- (instancetype)init {
    self = [super initWithEventId:kZAAppDeeplinkLaunchEvent];
    return self;
}
...
@end
@ZAAppLoadModule(ZADeeplinkManager)
@interface ZADeeplinkManager ()
...
@implementation ZADeeplinkManager

+ (instancetype)defaultManager {
    static dispatch_once_t onceToken;
    static ZADeeplinkManager *manager = nil;
    dispatch_once(&onceToken, ^{
        manager = [[ZADeeplinkManager alloc] init];
    });
    return manager;
}
...
- (void)filterValidSourceChannnels:(NSArray *)sourceChannels {
    NSSet *reservedPropertyName = zalldata_reserved_properties();
    ...
    for (NSString *name in sourceChannels) {
        if (![reservedPropertyName containsObject:name]) {
            ...
            // 这里只做 LOG 提醒
            ZALogError(@"deeplink source channel property [%@] is invalid!!!", name);
        }
    }
    _sourceChannels = set;
}
...
- (void)unarchiveSavedDeepLinkInfo:(BOOL)enableSave {
    if (!enableSave) {
        [ZAFileStore archiveWithFileName:kSavedDeepLinkInfoFileName value:nil];
        return;
    }
    NSDictionary *local = [ZAFileStore unarchiveWithFileName:kSavedDeepLinkInfoFileName];
    if (!local) {
        return;
    }
...
- (void)clearLastDeepLinkInfo:(NSURL *)url {
    ...
    self.utms[kZAEventPropertyDeepLinkURL] = url.absoluteString;
}
...
- (void)saveDeepLinkInfo:(NSDictionary *)dictionary {
    ...
    [ZAFileStore archiveWithFileName:kSavedDeepLinkInfoFileName value:dictionary];
}
...
// URL 的 Query 中包含一个或多个 utm_* 参数。示例：https://zalldata.cn?utm_content=1&utm_campaign=2
...
- (BOOL)checkDeepLinkMode:(NSURL *)url {
    ...
    if ([self isValidURLForServerMode:url]) {
        ...
        ZAEventDeepLinkLaunchObject *object = [[ZAEventDeepLinkLaunchObject alloc] init];
        NSMutableDictionary *properties = [NSMutableDictionary dictionary];
        properties[ZA_EVENT_PROPERTY_APP_INSTALL_SOURCE] = [self appInstallSource];
        [properties addEntriesFromDictionary:self.utms];
        [properties addEntriesFromDictionary:self.latestUtms];
        properties[kZAEventPropertyDeepLinkURL] = url.absoluteString;
        ...
        NSDictionary *dictionary = [ZAURLUtils queryItemsWithURL:url];
...
- (NSDictionary *)acquireLatestUtmProperties:(NSDictionary *)dictionary {
    ...
    // 用户自定义的属性，不是 SDK 的预置属性，因此以 _latest 开头，避免 ZA 平台报错。示例：_lateset_customKey
    for (NSString *propKey in _sourceChannels) {
...
/// URL 的 Path 符合特定规则。示例：https://{域名}/sd/{appId}/{key} 或 {scheme}://zalldata/sd/{key}
- (BOOL)isValidURLForServerMode:(NSURL *)url {
    NSArray *pathComponents = url.pathComponents;
    if (pathComponents.count < 2 || ![pathComponents[1] isEqualToString:@"sd"]) {
        return NO;
    }
    NSString *host = ZallDataSDK.sharedInstance.network.serverURL.host;
    return ([url.host isEqualToString:@"zalldata"] || [url.host isEqualToString:host]);
}
...
- (void)requestDeepLinkInfo:(NSURL *)url {
    ...
    NSURLSessionDataTask *task = [ZAHTTPSession.sharedInstance dataTaskWithRequest:request completionHandler:^(NSData *_Nullable data, NSHTTPURLResponse *_Nullable response, NSError *_Nullable error) {
        ...
        if (response.statusCode == 200 && data) {
            result = [ZAJSONUtil JSONObjectWithData:data];
...
- (void)trackDeepLinkLaunchWithURL:(NSString *)url {
    if (url && ![url isKindOfClass:NSString.class]) {
        ZALogError(@"deeplink url must be NSString. got: %@ %@", url.class, url);
        return;
    }
    ZAEventDeepLinkLaunchObject *object = [[ZAEventDeepLinkLaunchObject alloc] init];
    NSMutableDictionary *properties = [NSMutableDictionary dictionary];
    properties[kZAEventPropertyDeepLinkURL] = url;
    properties[ZA_EVENT_PROPERTY_APP_INSTALL_SOURCE] = [self appInstallSource];
    [ZallDataSDK.sharedInstance asyncTrackEventObject:object properties:properties];
}
...
- (void)trackDeeplinkMatchedResult:(NSURL *)url result:(NSDictionary *)result interval:(NSTimeInterval)interval errorMsg:(NSString *)errorMsg {
    ...
    props[kZAEventPropertyDeepLinkURL] = url.absoluteString;
    NSDictionary *utms = [self acquireUtmProperties:result[@"channel_params"]];
    [props addEntriesFromDictionary:utms];
    ZAEventPresetTrackObject *object = [[ZAEventPresetTrackObject alloc] initWithEventId:kZADeeplinkMatchedResultEvent];
    [ZallDataSDK.sharedInstance asyncTrackEventObject:object properties:props];
}
