...
#import "ZARSAEncryptor.h"
#import <Security/Security.h>
#import "ZALog.h"
#import "ZAUtilCheck.h"
@interface ZARSAEncryptor ()

@end

@implementation ZARSAEncryptor
...
- (NSString *)algorithm {
    return kZAAlgorithmTypeRSA;
}

- (NSString *)encryptData:(NSData *)data {
    if (za_check_empty_data(data)) {
        ZALogError(@"Enable RSA encryption but the input obj is invalid!");
        return nil;
    }

    NSString *asymmetricPublicKey = self.key;
    if (za_check_empty_string(asymmetricPublicKey)) {
        ZALogError(@"Enable RSA encryption but the public key is invalid!");
        return nil;
    }
    
    SecKeyRef keyRef = [self addPublicKey:asymmetricPublicKey];
    if (!keyRef) {
        ZALogError(@"Enable RSA encryption but init public SecKeyRef failed!");
        return nil;
    }
    ...
    for(int idx=0; idx<srclen; idx+=src_block_size) {
        ...
        if (status != 0) {
            ZALogError(@"SecKeyEncrypt fail. Error Code: %d", (int)status);
            ret = nil;
            break;
        }else{
