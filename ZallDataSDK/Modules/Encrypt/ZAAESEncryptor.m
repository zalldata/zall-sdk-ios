...
@interface ZAAESEncryptor ()

@property (nonatomic, copy, readwrite) NSData *key;

@end

@implementation ZAAESEncryptor
...
- (NSString *)algorithm {
    return kZAAlgorithmTypeAES;
}

- (nullable NSString *)encryptData:(NSData *)obj {
    if (za_check_empty_data(obj)) {
        ZALogError(@"Enable AES encryption but the input obj is invalid!");
        return nil;
    }

    if (za_check_empty_data(self.key)) {
        ZALogError(@"Enable AES encryption but the secret key data is invalid!");
        return nil;
    }
    ...
    if (cryptStatus == kCCSuccess) {
        ...
        free(buffer);
        ZALogError(@"AES encrypt data failed, with error Code: %d",(int)cryptStatus);
    }
    return nil;
}
