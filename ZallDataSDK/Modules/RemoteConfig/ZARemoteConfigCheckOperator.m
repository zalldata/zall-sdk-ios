...
typedef void (^ ZARemoteConfigCheckAlertHandler)(ZAAlertAction *action);

@interface ZARemoteConfigCheckAlertModel : NSObject

@property (nonatomic, copy) NSString *title;
@property (nonatomic, copy) NSString *message;
@property (nonatomic, copy) NSString *defaultStyleTitle;
@property (nonatomic, copy) ZARemoteConfigCheckAlertHandler defaultStyleHandler;
@property (nonatomic, copy) NSString *cancelStyleTitle;
@property (nonatomic, copy) ZARemoteConfigCheckAlertHandler cancelStyleHandler;

@end

@implementation ZARemoteConfigCheckAlertModel
...
@interface ZARemoteConfigCheckOperator ()
...
@implementation ZARemoteConfigCheckOperator

#pragma mark - Protocol

- (BOOL)handleRemoteConfigURL:(NSURL *)url {
    ZALogDebug(@"【remote config】The input QR url is: %@", url);
    ...
    NSDictionary *components = [ZAURLUtils queryItemsWithURL:url];
    if (!components) {
        ZALogError(@"【remote config】The QR url format is invalid");
        return NO;
    }
    ...
    ZALogDebug(@"【remote config】The input QR url project is %@, os is %@, app_id is %@", urlProject, urlOS, urlAppID);
    ...
    ZALogDebug(@"【remote config】The current project is %@, os is %@, app_id is %@", currentProject, currentOS, currentAppID);
...
- (void)showNetworkErrorAlert {
    ZARemoteConfigCheckAlertModel *model = [[ZARemoteConfigCheckAlertModel alloc] init];
...
- (void)showURLCheckAlertWithMessage:(NSString *)message isCheckPassed:(BOOL)isCheckPassed urlVersion:(NSString *)urlVersion {
    ...
    if (isCheckPassed) {
        ...
        model.defaultStyleHandler = ^(ZAAlertAction *action) {
...
- (void)showAlertWithModel:(ZARemoteConfigCheckAlertModel *)model {
    za_quick_dispatch_sync_on_main_queue(^{
        ZAAlertViewController *alertController = [[ZAAlertViewController alloc] initWithTitle:model.title message:model.message preferredStyle:ZAAlertControllerStyleAlert];
        [alertController addActionWithTitle:model.defaultStyleTitle style:ZAAlertActionStyleDefault handler:model.defaultStyleHandler];
        if (model.cancelStyleTitle) {
            [alertController addActionWithTitle:model.cancelStyleTitle style:ZAAlertActionStyleCancel handler:model.cancelStyleHandler];
        }
    ...
    [self requestRemoteConfigWithForceUpdate:YES completion:^(BOOL success, NSDictionary<NSString *,id> * _Nullable config) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            @try {
                ...
                ZALogError(@"【remote config】%@ error: %@", strongSelf, exception);
            }
        });
    }];
}
...
- (void)handleRemoteConfig:(NSDictionary<NSString *, id> *)remoteConfig withURLVersion:(NSString *)urlVersion {
    ...
    enableMDic[@"localLibVersion"] = ZallDataSDK.libVersion;
...
- (void)showIndicator {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        UIViewController *controller = [[ZAAlertViewController alloc] init];
