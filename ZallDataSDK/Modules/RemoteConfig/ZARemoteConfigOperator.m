...
#import "ZARemoteConfigEventObject.h"
...
@interface ZARemoteConfigOperator ()
...
@implementation ZARemoteConfigOperator
...
- (BOOL)isBlackListContainsEvent:(nullable NSString *)event {
    if (za_check_empty_string(event)) {
        return NO;
    }
    ...
    @try {
        ...
        NSURLSessionDataTask *task = [ZAHTTPSession.sharedInstance dataTaskWithRequest:request completionHandler:^(NSData * _Nullable data, NSHTTPURLResponse * _Nullable response, NSError * _Nullable error) {
            ...
            if (statusCode == 200 && data.length) {
                config = [ZAJSONUtil JSONObjectWithData:data];
            }
        ...
        ZALogError(@"【remote config】%@ error: %@", self, e);
        completion(NO, nil);
    }
}
...
- (void)trackAppRemoteConfigChanged:(NSDictionary<NSString *, id> *)remoteConfig {
    NSString *eventConfigString = [ZAJSONUtil stringWithJSONObject:remoteConfig];
    ZARemoteConfigEventObject *object = [[ZARemoteConfigEventObject alloc] initWithEventId:kZAEventNameAppRemoteConfigChanged];
    [ZallDataSDK.sdkInstance asyncTrackEventObject:object properties:@{kZAEventPropertyAppRemoteConfig : eventConfigString ?: @""}];
    // 触发 $AppRemoteConfigChanged 时 flush 一次
    [ZallDataSDK.sdkInstance trackForceSendAll];
}

- (void)enableRemoteConfig:(NSDictionary *)config {
    self.model = [[ZARemoteConfigModel alloc] initWithDictionary:config];
    
    /* 发送远程配置模块 Model 变化通知
     定位和设备方向采集（Location 和 DeviceOrientation 模块），依赖这个通知，如果使用相关功能，必须开启远程控制功能
    */
    [[NSNotificationCenter defaultCenter] postNotificationName:ZA_REMOTE_CONFIG_MODEL_CHANGED_NOTIFICATION object:self.model];
}

#pragma mark Network

- (BOOL)isLibVersionUnchanged {
    return [self.model.localLibVersion isEqualToString:[ZallDataSDK libVersion]];
}
...
#endif
    return ZAModuleManager.sharedInstance.hasSecretKey;
}
...
- (NSURLRequest *)buildURLRequestWithOriginalVersion:(nullable NSString *)originalVersion latestVersion:(nullable NSString *)latestVersion {
    ...
    if (!urlComponets.host) {
        ...
        if (!urlComponets.host) {
            ZALogError(@"【remote config】URLString is malformed, nil is returned.");
            return nil;
        }
...
- (NSString *)buildQueryWithURL:(NSURL *)url originalVersion:(NSString *)originalVersion latestVersion:(NSString *)latestVersion {
    NSDictionary *originalParams = [ZAURLUtils queryItemsWithURL:url];
    ...
    return [ZAURLUtils urlQueryStringWithParams:params];
}
...
- (NSString *)project {
    return [ZAURLUtils queryItemsWithURL:self.serverURL][@"project"];
}
