...
static void ZAFree(void *p) {
    if (p) {
        free(p);
    }
}

static void *ZAAllocBufferForObjCType(const char *objCType) {
    void *buffer = NULL;
    ...
    if (result != 0) {
        ZALogError(@"Error allocating aligned memory: %s", strerror(result));
    }
...
@implementation NSInvocation (ZAHelpers)

- (void)za_setArgument:(id)argumentValue atIndex:(NSUInteger)index {
    ...
    if ([argumentValue isKindOfClass:[NSNumber class]] && strnlen(argumentType, 8) == 1) {
        ...
        void *buffer = ZAAllocBufferForObjCType([valueArgument objCType]);
...
-(void)za_setArgumentsFromArray: (NSArray *)argumentArray {
    ...
    for (NSUInteger i = 0; i < [argumentArray count]; ++i) {
        ...
        [self za_setArgument:argumentArray[i] atIndex:argumentIndex];
    }
}
...
- (id)za_returnValue {
    ...
    void *buffer = ZAAllocBufferForObjCType(objCType);
