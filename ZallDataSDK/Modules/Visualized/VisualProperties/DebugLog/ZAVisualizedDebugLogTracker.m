...
@interface ZAVisualizedDebugLogTracker()<ZAVisualizedLoggerDelegate>
@property (atomic, strong, readwrite) NSMutableArray<NSMutableDictionary *> *debugLogInfos;
@property (nonatomic, strong) ZAVisualizedLogger *logger;
...
@implementation ZAVisualizedDebugLogTracker

- (instancetype)init {
    self = [super init];
    if (self) {
        NSString *serialQueueLabel = [NSString stringWithFormat:@"com.zalldata.ZAVisualizedDebugLogTracker.%p", self];
...
- (void)addDebugLogger {
    // 添加 log 实现
    ZAVisualizedLogger *visualizedLogger = [[ZAVisualizedLogger alloc] init];
    [ZALog addLogger:visualizedLogger];

    visualizedLogger.delegate = self;
    self.logger = visualizedLogger;
}

#pragma mark ZAVisualizedLoggerDelegate
...
- (void)addAllNodeInfo {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        ZAViewNode *rootNode = keyWindow.zalldata_viewNode.nextNode;
        ...
        dispatch_async(self.serialQueue, ^{
            ...
            @try {
                ...
                NSString *logMessage = [ZAVisualizedLogger buildLoggerMessageWithTitle:@"诊断信息" message:@"log node tree error: %@", exception];
...
// 显示每层 node 的信息
- (NSString *)showViewHierarchy:(ZAViewNode *)node level:(NSInteger)level {
     ...
     */
    for (ZAViewNode *node1 in [node.subNodes copy]) {
...
- (void)dealloc {
    // 移除注入的 logger
    [ZALog removeLogger:self.logger];
}
