...
/// 日志过滤前缀
static NSString * const kZAVisualizedLoggerPrefix = @"ZAVisualizedDebugLoggerPrefix:";

/// 日志的 title 和 messsage 分隔符
static NSString * const kZAVisualizedLoggerSeparatedChar = @"：";

@implementation ZALoggerVisualizedFormatter

- (NSString *)formattedLogMessage:(ZALogMessage *)logMessage {
    return logMessage.message;
}
@end


@implementation ZAVisualizedLogger

- (instancetype)init {
    self = [super init];
    if (self) {
        self.loggerQueue = dispatch_queue_create("cn.zalldata.ZAVisualizedLoggerSerialQueue", DISPATCH_QUEUE_SERIAL);
    }
    return self;
}
...
- (void)logMessage:(ZALogMessage *)logMessage {
    ...
    ZALoggerVisualizedFormatter *formatter = [[ZALoggerVisualizedFormatter alloc] init];

    // 获取日志
    NSString *message = [formatter formattedLogMessage:logMessage];

    // 筛选自定义属性日志
    if (![message containsString:kZAVisualizedLoggerPrefix]) {
        return;
    }
    NSRange range = [message rangeOfString:kZAVisualizedLoggerPrefix];

    NSString *debugLog = [message substringFromIndex:range.location + range.length];

    // 格式校验
    if (![debugLog containsString:kZAVisualizedLoggerSeparatedChar]) {
        return;
    }

    NSRange separatedRange = [debugLog rangeOfString:kZAVisualizedLoggerSeparatedChar];
...
#pragma mark -
@implementation ZAVisualizedLogger (Build)

+ (NSString *)buildLoggerMessageWithTitle:(NSString *)title message:(NSString *)format, ... {
    NSMutableString *logMessage = [NSMutableString stringWithString:kZAVisualizedLoggerPrefix];
    if (title) { // 拼接标题
        [logMessage appendString:title];
        [logMessage appendString:kZAVisualizedLoggerSeparatedChar];
    }
