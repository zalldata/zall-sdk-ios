...
- (void)buildNodeRelation {
    ...
    for (UIView *subView in view.subviews) {
        if (subView.zalldata_viewNode) {
            // 可能存在其他父节点，进行移除
            if (subView.zalldata_viewNode.nextNode && subView.zalldata_viewNode.nextNode != self) {
                [subView.zalldata_viewNode removeOldNodeRelation];
            }
            [subView.zalldata_viewNode buildNextNodeRelationWithNextNode:self];
        }
    }
    ...
    ZAViewNode *nextNode = nextView.zalldata_viewNode;
    if (nextNode) {
...
- (void)buildNextNodeRelationWithNextNode:(ZAViewNode *)nextNode {
    // 链接父节点
...
- (void)removeOldNodeRelation {
    ZAViewNode *oldNextNode = self.nextNode;
...
- (void)refreshIndex {
    ...
    NSInteger index = 0;
    for (ZAViewNode *node in self.nextNode.subNodes) {
        if (node == self) {
            self.index = index;
            return;
        }
...
- (void)refreshScreenName {
    za_quick_dispatch_sync_on_main_queue(^{
        self.screenName = [self.view zalldata_screenName];
    });
}
...
- (void)refreshSubNodeScreenName {
    ...
    for (ZAViewNode *subNode in self.subNodes) {
...
- (NSString *)elementPosition {
    ZAViewNode *nextNode = self.nextNode;
...
- (NSString *)elementPath {
    ...
    可以在子线程执行
    实现参考 [ZAVisualizedUtils viewSimilarPathForView: atViewController: shouldSimilarPath:]
     */
    ZAViewNode *currentNode = self;
...
- (NSString *)elementPath {
    ZAViewNode *segmentedControlNode = self.nextNode;
...
@interface ZASegmentedControlNode()
@property (nonatomic, strong) UISegmentedControl *segmentedControl;
@end

@implementation ZASegmentedControlNode
...
@implementation ZATabBarButtonNode
...
@implementation ZATableViewHeaderFooterViewNode
...
@interface ZACellNode()
@property (nonatomic, strong) NSIndexPath *indexPath;

@end

@implementation ZACellNode
...
-(NSIndexPath *)indexPath {
    if (!_indexPath) {
        za_quick_dispatch_sync_on_main_queue(^{
            if ([ZAVisualizedUtils isVisibleForView:self.view]) {
                [self refreshIndex];
            }
        });
    }
...
- (NSString *)similarPath {
    if (self.indexPath) {
        // 弹框或 menu 等，不支持限定位置
        ZAViewElementInfo *elementInfo = [ZAViewElementInfoFactory elementInfoWithView:self.view];
...
@implementation ZARNViewNode
- (instancetype)initWithView:(UIView *)view {
    [ZARNViewNode bindScreenNameWithClickableView:view];
    self = [super initWithView:view];
    return self;
}
...
+ (void)bindScreenNameWithClickableView:(UIView *)rnView {
    [rnView zalldata_clickableForRNView];
}

@end


@implementation ZAWKWebViewNode
...
- (void)callJSSendVisualConfig:(NSDictionary *)configResponse {
    ...
    // 判断打通才注入配置
    if (![ZAVisualizedUtils isSupportCallJSWithWebView:webView]) {
        return;
    }
    // 调用 JS 函数，发送配置信息
    NSString *javaScriptSource = [ZAJavaScriptBridgeBuilder buildCallJSMethodStringWithType:ZAJavaScriptCallJSTypeUpdateVisualConfig jsonObject:configResponse];
    if (!javaScriptSource) {
        return;
    }
    [webView evaluateJavaScript:javaScriptSource completionHandler:^(id _Nullable resuts, NSError * _Nullable error) {
        if (error) {
            ZALogDebug(@"%@ updateH5VisualConfig error: %@", kZAJSBridgeCallMethod, error);
        } else {
            ZALogDebug(@"%@ updateH5VisualConfig finish", kZAJSBridgeCallMethod);
        }
    }];
}
...
@implementation ZAIgnorePathNode
