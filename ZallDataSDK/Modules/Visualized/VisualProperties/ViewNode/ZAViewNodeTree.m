...
static void * const kZARNManagerContext = (void*)&kZARNManagerContext;
static NSString * const kZARNManagerScreenPropertiesKeyPath = @"screenProperties";

@interface ZAViewNodeTree()

/// 当前根节点
@property (nonatomic, strong) ZAViewNode *rootNode;

/// 自定义属性采集队列
@property (nonatomic, strong) dispatch_queue_t serialQueue;
@end

@implementation ZAViewNodeTree
...
- (void)initialization {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        UIWindow *keyWindow = [ZAVisualizedUtils currentValidKeyWindow];
        ZAViewNode *keyWindowNode = [ZAViewNodeFactory viewNodeWithView:keyWindow];

        // 为了支持多 window，此处构建虚拟根节点
        UIView *rootView = [[UIView alloc] initWithFrame:keyWindow.bounds];
        ZAViewNode *rootNode = [[ZAViewNode alloc] initWithView:rootView];
...
- (void)setupListeners {
    ...
    dispatch_once(&onceToken, ^{
         ...
         */
        NSError *error = nil;
        [UIView za_swizzleMethod:@selector(didMoveToSuperview) withMethod:@selector(zalldata_visualize_didMoveToSuperview) error:&error];
        if (error) {
            ZALogError(@"Failed to swizzle on UIView. Error details: %@", error);
        }
        
        // 测试发现部分场景下，UINavigationTransitionView 未执行 didMoveToSuperview，但是执行了 didMoveToWindow
        [UIView za_swizzleMethod:@selector(didMoveToWindow) withMethod:@selector(zalldata_visualize_didMoveToWindow) error:NULL];
        
        // 测试发现 UIAlertController.view 即 _UIAlertControllerView 显示，未执行 didMoveToWindow 和 didMoveToSuperview，但是其父视图调用了 didAddSubview
        [UIView za_swizzleMethod:@selector(didAddSubview:) withMethod:@selector(zalldata_visualize_didAddSubview:) error:NULL];
        
        // 兼容在业务中，动态修改了 keyWindow
        [UIWindow za_swizzleMethod:@selector(becomeKeyWindow) withMethod:@selector(zalldata_visualize_becomeKeyWindow) error:NULL];
        
        // 针对 tab 元素，调用 setSelectedItem 切换页面后，更新子视图页面名称
        [UITabBar za_swizzleMethod:@selector(setSelectedItem:) withMethod:@selector(zalldata_visualize_setSelectedItem:) error:NULL];

        // 兼容 RN 项目，tab 点击触发页面浏览事件，但是 tab 对应节点的 screenName 为更新，监听 screenProperties 设置，并更新 RN 节点的页面信息
        Class rnManagerClass = NSClassFromString(@"ZAReactNativeManager");
        if (rnManagerClass) {
...
#pragma clang diagnostic ignored "-Warc-performSelector-leaks"
                ...
                [manager addObserver:self forKeyPath:kZARNManagerScreenPropertiesKeyPath options:(NSKeyValueObservingOptionNew|NSKeyValueObservingOptionOld) context:kZARNManagerContext];
#pragma clang diagnostic pop
            }
        }

        // bringSubviewToFront 和 sendSubviewToBack，不执行 didMoveTo 相关方法，但是会修改 index，从而改变路径
        [UIView za_swizzleMethod:@selector(bringSubviewToFront:) withMethod:@selector(zalldata_visualize_bringSubviewToFront:) error:NULL];
        
        [UIView za_swizzleMethod:@selector(sendSubviewToBack:) withMethod:@selector(zalldata_visualize_sendSubviewToBack:) error:NULL];
        
        // cell 被重用，需要重新计算 indexPath
        [UITableViewCell za_swizzleMethod:@selector(prepareForReuse) withMethod:@selector(zalldata_visualize_prepareForReuse) error:NULL];
        
        [UICollectionViewCell za_swizzleMethod:@selector(prepareForReuse) withMethod:@selector(zalldata_visualize_prepareForReuse) error:NULL];
        
        // HeaderFooterView 被重用，重新计算 index
        [UITableViewHeaderFooterView za_swizzleMethod:@selector(prepareForReuse) withMethod:@selector(zalldata_visualize_prepareForReuse) error:NULL];
    });
}
...
- (void)becomeKeyWindow:(UIWindow *)window {
    dispatch_async(dispatch_get_main_queue(), ^{
         ...
         */
        for (ZAViewNode *node in self.rootNode.subNodes) {
            if (node.view == window) {
                return;
            }
        }
        ...
        // 构建链接关系
        ZAViewNode *viewNode = [ZAViewNodeFactory viewNodeWithView:window];
...
- (void)removeViewNodeWithView:(UIView *)view {
    if (view.superview || !view.zalldata_viewNode) {
        return;
    }
...
- (void)addViewNodeWithView:(UIView *)view {
    ...
    ZAViewNode *node = [ZAViewNodeFactory viewNodeWithView:view];
    ...
    for (ZAViewNode *subNode in nextView.zalldata_viewNode.subNodes) {
...
- (BOOL)isIgnoreBuildNodeWithView:(UIView *)view {
    ...
    if ([ZAVisualizedUtils isIgnoreSubviewsWithView:superView] || [ZAVisualizedUtils isIgnoreSubviewsWithView:nextView]) {
        return YES;
    }
    return NO;
}
...
- (void)updateBrotherViewNodeWithView:(UIView *)view isAddViewNode:(BOOL)isAdd {
    ZAViewNode *currentNode = view.zalldata_viewNode;
...
- (void)refreshRNViewScreenNameWithViewController:(UIViewController *)viewController {
     ...
     */
    dispatch_async(dispatch_get_main_queue(), ^{
        ZAViewNode *viewNode = viewController.view.zalldata_viewNode;
        [viewNode refreshSubNodeScreenName];
    });
}

- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    if (context != kZARNManagerContext) {
        return [super observeValueForKeyPath:keyPath ofObject:object change:change context:context];
    }
    if (![keyPath isEqualToString:kZARNManagerScreenPropertiesKeyPath]) {
        return;
    }
    
    NSString *oldScreenName = nil;
    if ([change[NSKeyValueChangeOldKey] isKindOfClass:NSDictionary.class]) {
        oldScreenName = change[NSKeyValueChangeOldKey][kZAEventPropertyScreenName];
    }
    NSString *newScreenName = change[NSKeyValueChangeNewKey][kZAEventPropertyScreenName];
    ...
    /* RN 元素自定义属性兼容逻辑
     ...
     同时在 trackViewScreen 中，会将 ZAReactNativeManager 单例中保存的页面名称 (visualizeProperties 接口返回) 更新为 B
    ...
    __block ZAViewNode *windowNode = nil;
    ...
    dispatch_async(self.serialQueue, ^{
        // zalldata_clickableForRNView 等操作，需要主线程执行
        dispatch_async(dispatch_get_main_queue(), ^{
...
// 刷新 RN 元素节点页面名称
- (void)refreshRNViewScreenName:(NSString *)screenName viewNode:(ZAViewNode *)viewNode {
    if ([viewNode isKindOfClass:ZARNViewNode.class] && [viewNode.view zalldata_clickableForRNView] && ![screenName isEqualToString:viewNode.screenName]) {
        [viewNode refreshSubNodeScreenName];
        return;
    }
    for (ZAViewNode *subNode in viewNode.subNodes) {
        [self refreshRNViewScreenName:screenName viewNode:subNode];
    }
}

#pragma mark queryView
- (UIView *)viewWithPropertyConfig:(ZAVisualPropertiesPropertyConfig *)config {
    return [self viewWithPropertyConfig:config viewNode:self.rootNode];
}

- (UIView *)viewWithPropertyConfig:(ZAVisualPropertiesPropertyConfig *)config viewNode:(ZAViewNode *)node {
    if ([config isMatchVisualPropertiesWithViewIdentify:node]) {
        return node.view;
    }
    __block UIView *resultView = nil;
    [node.subNodes enumerateObjectsUsingBlock:^(ZAViewNode *_Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
...
- (void)sendWebViewConfig:(NSDictionary *)configResponse viewNode:(ZAViewNode *)node {
    if ([node isKindOfClass:ZAWKWebViewNode.class]) {
        ZAWKWebViewNode *webViewNode = (ZAWKWebViewNode *)node;
        
        // getWindow 需要在主线程执行
        dispatch_async(dispatch_get_main_queue(), ^{
            // 判断 WebView 是否显示
            if (!webViewNode.view.window || ![ZAVisualizedUtils isVisibleForView:webViewNode.view]) {
                return;
            }
