//
// ZAVisualizedObjectSerializerManager.m
// ZallDataSDK
//
// Created by guo on 2020/4/23.
...
@implementation ZAVisualizedWebPageInfo

@end


@interface ZAVisualizedObjectSerializerManager()

/// 是否包含 webview
@property (nonatomic, assign, readwrite) BOOL isContainWebView;

/// App 内嵌 H5 页面信息
@property (nonatomic, strong, readwrite) ZAVisualizedWebPageInfo *webPageInfo;
...
/*
 key:H5 页面 url
 value:ZAVisualizedWebPageInfo 对象
 */
@property (nonatomic, strong) NSMutableDictionary <NSString *,ZAVisualizedWebPageInfo *>*webPageInfoCache;

@end

@implementation ZAVisualizedObjectSerializerManager

+ (instancetype)sharedInstance {
    static ZAVisualizedObjectSerializerManager *manager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        manager = [[ZAVisualizedObjectSerializerManager alloc] init];
    });
    return manager;
}
...
- (void)saveWebElementInfoWithData:(NSMutableDictionary *)pageInfo webView:(WKWebView *)webview {
    ...
    ZAVisualizedWebPageInfo *webPageInfo = nil;
    // 是否包含当前 url 的页面信息
    if ([self.webPageInfoCache objectForKey:url]) {
        ...
        webPageInfo = [[ZAVisualizedWebPageInfo alloc] init];
...
/// 读取当前 webView 页面相关信息
- (ZAVisualizedWebPageInfo *)readWebPageInfoWithWebView:(WKWebView *)webView {
    NSString *url = webView.URL.absoluteString;
    ZAVisualizedWebPageInfo *webPageInfo = [self.webPageInfoCache objectForKey:url];
    return webPageInfo;
}

- (void)enterWebViewPageWithWebInfo:(ZAVisualizedWebPageInfo *)webInfo; {
...
- (void)refreshPayloadHashWithData:(id)obj {
    ...
    if (jsonData) {
        // 计算 hash
        self.jointPayloadHash = [ZAQuickUtil hashStringWithData:jsonData];
    }
}
