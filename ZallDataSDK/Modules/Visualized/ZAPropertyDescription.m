...
@implementation ZAPropertySelectorParameterDescription
...
@implementation ZAPropertySelectorDescription
...
@interface ZAPropertyDescription ()

@end

@implementation ZAPropertyDescription

+ (NSValueTransformer *)valueTransformerForType:(NSString *)typeName {
    for (NSString *toTypeName in @[@"NSDictionary", @"NSNumber", @"NSString"]) {
        NSString *toTransformerName = [NSString stringWithFormat:@"ZA%@To%@ValueTransformer", typeName, toTypeName];
    ...
    return [NSValueTransformer valueTransformerForName:@"ZAPassThroughValueTransformer"];
}
...
- (instancetype)initWithDictionary:(NSDictionary *)dictionary {
    ...
    if (self) {
        ...
        _getSelectorDescription = [[ZAPropertySelectorDescription alloc] initWithDictionary:get];
