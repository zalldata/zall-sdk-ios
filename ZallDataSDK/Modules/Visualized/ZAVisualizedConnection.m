...
@interface ZAVisualizedConnection ()
@property (nonatomic, strong) NSTimer *timer;
@end

@implementation ZAVisualizedConnection {
    BOOL _connected;
    NSDictionary *_typeToMessageClassMap;
    NSOperationQueue *_commandQueue;
    id<ZAVisualizedMessage> _designerMessage;
...
- (instancetype)init {
    ...
    if (self) {
        _typeToMessageClassMap = @{
            ZAVisualizedSnapshotRequestMessageType : [ZAVisualizedSnapshotRequestMessage class],
        };
        _connected = NO;
...
- (void)setUpListeners {
    ...
    [notificationCenter addObserver:self selector:@selector(receiveVisualizedMessageFromH5:) name:ZA_VISUALIZED_H5_MESSAGE_NOTIFICATION object:nil];
}
...
- (void)receiveVisualizedMessageFromH5:(NSNotification *)notification {
    ...
    if (![webView isKindOfClass:WKWebView.class]) {
        ZALogError(@"Message webview is invalid from JS SDK");
        return;
    }
    
    NSMutableDictionary *messageDic = [ZAJSONUtil JSONObjectWithString:message.body options:NSJSONReadingMutableContainers];
    if (![messageDic isKindOfClass:[NSDictionary class]]) {
        ZALogError(@"Message body is formatted failure from JS SDK");
        return;
    }
    
    [[ZAVisualizedObjectSerializerManager sharedInstance] saveVisualizedWebPageInfoWithWebView:webView webPageInfo:messageDic];
}
...
- (void)close {
    ...
    [[ZAVisualizedObjectSerializerManager sharedInstance] cleanVisualizedWebPageInfoCache];

    // 关闭埋点校验
    [ZAVisualizedManager.defaultManager enableEventCheck:NO];

    // 关闭诊断信息收集
    [ZAVisualizedManager.defaultManager.visualPropertiesTracker enableCollectDebugLog:NO];
}
...
- (void)sendMessage:(id<ZAVisualizedMessage>)message {
    if (_connected) {
        ...
        NSURLSessionDataTask *task = [ZAHTTPSession.sharedInstance dataTaskWithRequest:request completionHandler:^(NSData *_Nullable data, NSHTTPURLResponse *_Nullable response, NSError *_Nullable error) {
            ...
            if (response.statusCode == 200) {
                NSDictionary *dict = [ZAJSONUtil JSONObjectWithString:urlResponseContent];
                int delay = [dict[@"delay"] intValue];
                if (delay < 0) {
                    [self close];
                }
                
                // 切到主线程，和 ZAVisualizedManager 中调用一致
        ...
        ZALogWarn(@"Not sending message as we are not connected: %@", [message debugDescription]);
    }
}
...
- (void)analysisDebugMessage:(NSDictionary *)message {
    // 关闭自定义属性也不再处理调试信息
    if (message.count == 0 || !ZAVisualizedManager.defaultManager.configOptions.enableVisualizedProperties) {
        return;
    }
    ...
    if (disableConfig) {
        NSString *logMessage = [ZAVisualizedLogger buildLoggerMessageWithTitle:@"开关控制" message:@"轮询接口返回，运维配置，关闭自定义属性"];
        ZALogDebug(@"%@", logMessage);

        [ZAVisualizedManager.defaultManager.configSources setupConfigWithDictionary:nil disableConfig:YES];
    } else if (configDic.count > 0) {
        NSString *logMessage = [ZAVisualizedLogger buildLoggerMessageWithTitle:@"获取配置" message:@"轮询接口更新可视化全埋点配置，%@", configDic];
        ZALogInfo(@"%@", logMessage);

        [ZAVisualizedManager.defaultManager.configSources setupConfigWithDictionary:configDic disableConfig:NO];
    }

    // 前端页面进入 &debug=1 调试模式
    BOOL isDebug = [message[@"visualized_debug_mode_enabled"] boolValue];
    [ZAVisualizedManager.defaultManager.visualPropertiesTracker enableCollectDebugLog:isDebug];
}


- (id <ZAVisualizedMessage>)designerMessageForMessage:(NSString *)message {
    if (![message isKindOfClass:[NSString class]]) {
        ZALogError(@"message type error:%@",message);
        return nil;
    }

    id jsonObject = [ZAJSONUtil JSONObjectWithString:message];
    if (![jsonObject isKindOfClass:[NSDictionary class]]) {
        ZALogError(@"Badly formed socket message expected JSON dictionary: %@", message);
        return nil;
    }
    ...
    id <ZAVisualizedMessage> designerMessage = [_typeToMessageClassMap[type] messageWithType:type payload:payload];
...
- (void)startConnectionWithFeatureCode:(NSString *)featureCode url:(NSString *)urlStr {
    NSString *jsonPath = [ZAQuickUtil zaJsonPathBundleWithName:@"za_visualized_path"];
