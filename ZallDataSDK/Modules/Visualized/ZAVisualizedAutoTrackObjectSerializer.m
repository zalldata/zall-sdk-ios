//
//  ZAObjectSerializer.m
//  ZallDataSDK
//
//  Created by guo on 1/18/16.
...
#import "NSInvocation+ZAHelpers.h"
#import "ZAClassDescription.h"
#import "ZALog.h"
#import "ZAObjectIdentityProvider.h"
#import "ZAVisualizedAutoTrackObjectSerializer.h"
#import "ZAObjectSerializerConfig.h"
#import "ZAObjectSerializerContext.h"
#import "ZAPropertyDescription.h"
...
@interface ZAVisualizedAutoTrackObjectSerializer ()
@end

@implementation ZAVisualizedAutoTrackObjectSerializer {
    ZAObjectSerializerConfig *_configuration;
    ZAObjectIdentityProvider *_objectIdentityProvider;
}

- (instancetype)initWithConfiguration:(ZAObjectSerializerConfig *)configuration
               objectIdentityProvider:(ZAObjectIdentityProvider *)objectIdentityProvider {
    self = [super init];
    if (self) {
...
- (NSDictionary *)serializedObjectsWithRootObject:(id)rootObject {
    ...
    ZAObjectSerializerContext *context = [[ZAObjectSerializerContext alloc] initWithRootObject:rootObject];
    ...
    @try {// 遍历 _unvisitedObjects 中所有元素，解析元素信息
        ...
        ZALogError(@"Failed to serialize objects: %@", e);
    }
...
- (void)visitObject:(NSObject *)object withContext:(ZAObjectSerializerContext *)context {
    ...
    ZAClassDescription *classDescription = [self classDescriptionForObject:object];
    if (classDescription) {
        // 遍历自身和父类的所需的属性及类型，合并为当前类所有属性
        for (ZAPropertyDescription *propertyDescription in [classDescription propertyDescriptions]) {
...
#pragma clang diagnostic pop
    ...
    if ([object isKindOfClass:ZAWebElementView.class]) {
        ZAWebElementView *touchView = (ZAWebElementView *)object;
...
- (NSInvocation *)invocationForObject:(id)object
              withSelectorDescription:(ZAPropertySelectorDescription *)selectorDescription {
...
propertyDescription:(ZAPropertyDescription *)propertyDescription
            context:(ZAObjectSerializerContext *)context {
...
- (id)propertyValueForObject:(NSObject *)object
     withPropertyDescription:(ZAPropertyDescription *)propertyDescription
                     context:(ZAObjectSerializerContext *)context {
    ZAPropertySelectorDescription *selectorDescription = propertyDescription.getSelectorDescription;
    ...
    if (propertyDescription.useKeyValueCoding) {
        ...
        if (invocation) {
            [invocation za_setArgumentsFromArray:@[]];
            [invocation invokeWithTarget:object];
            
            id returnValue = [invocation za_returnValue];
...
- (ZAClassDescription *)classDescriptionForObject:(NSObject *)object {
    NSParameterAssert(object != nil);
    
    Class aClass = [object class];
    while (aClass != nil) {
        ZAClassDescription *classDescription = [_configuration classWithName:NSStringFromClass(aClass)];
        if (classDescription) {
...
- (void)addNotWKWebViewAlertInfo {
    [[ZAVisualizedObjectSerializerManager sharedInstance] enterWebViewPageWithWebInfo:nil];
    ...
    alertInfo[@"link_url"] = @"https://manual.zalldata.cn/sa/latest/enable_visualized_autotrack-7548675.html";
    if ([ZAVisualizedManager defaultManager].visualizedType == ZallAnalyticsVisualizedTypeHeatMap) {
        alertInfo[@"title"] = @"当前页面无法进行点击分析";
        alertInfo[@"message"] = @"此页面包含 UIWebView，iOS App 内嵌 H5 点击分析，只支持 WKWebView";
        alertInfo[@"link_url"] = @"https://manual.zalldata.cn/sa/latest/app-16286049.html";
    }
    [[ZAVisualizedObjectSerializerManager sharedInstance] registWebAlertInfos:@[alertInfo]];
}
...
- (void)checkWKWebViewInfoWithWebView:(WKWebView *)webView {
    ...
    if (webPageInfo.alertSources) {
        [[ZAVisualizedObjectSerializerManager sharedInstance] registWebAlertInfos:webPageInfo.alertSources];
    }
    
    // H5 页面元素信息
    if (webPageInfo) {
        [[ZAVisualizedObjectSerializerManager sharedInstance] enterWebViewPageWithWebInfo:webPageInfo];
    }
    ...
    if (!webPageInfo) {
        ...
        [userScripts enumerateObjectsUsingBlock:^(WKUserScript *_Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
            if ([obj.source containsString:kZAJSBridgeVisualizedMode]) {
        ...
        if (isContainVisualized) {
            ...
            NSString *visualizedMode = [ZAJavaScriptBridgeBuilder buildVisualBridgeWithVisualizedMode:YES];
     ...
     */
    NSString *jsMethodString = [ZAJavaScriptBridgeBuilder buildCallJSMethodStringWithType:ZAJavaScriptCallJSTypeVisualized jsonObject:nil];
    ...
    [webView evaluateJavaScript:javaScriptSource completionHandler:^(id _Nullable response, NSError *_Nullable error) {
        if (error) {
             ...
             */
            ZALogDebug(@"window.sensorsdata_app_call_js error：%@", error);
        }
    }];
}
...
- (void)checkJSSDKIntegrationWithWebView:(WKWebView *)webView {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        // 延迟判断是否存在 js 发送数据
        ZAVisualizedWebPageInfo *currentWebPageInfo = [[ZAVisualizedObjectSerializerManager sharedInstance] readWebPageInfoWithWebView:webView];
        if (currentWebPageInfo) {
            return;
        }
        // 注入了 bridge 但是未接收到数据
        NSString *javaScript = [ZAJavaScriptBridgeBuilder buildCallJSMethodStringWithType:ZAJavaScriptCallJSTypeCheckJSSDK jsonObject:nil];
        [webView evaluateJavaScript:javaScript completionHandler:^(id _Nullable response, NSError *_Nullable error) {
            ...
            if (exceptionMessage && [exceptionMessage containsString:@"undefined is not a function"]) {
                ...
                alertInfo[@"link_text"] = @"配置文档";
                alertInfo[@"link_url"] = @"https://manual.zalldata.cn/sa/latest/tech_sdk_client_web_use-7548173.html";
                if ([ZAVisualizedManager defaultManager].visualizedType == ZallAnalyticsVisualizedTypeHeatMap) {
                    alertInfo[@"title"] = @"当前页面无法进行点击分析";
                }
                NSMutableDictionary *alertInfoMessage = [@{ @"callType": @"app_alert", @"data": @[alertInfo] } mutableCopy];
                [[ZAVisualizedObjectSerializerManager sharedInstance] saveVisualizedWebPageInfoWithWebView:webView webPageInfo:alertInfoMessage];
            }
        }];
    });
}
