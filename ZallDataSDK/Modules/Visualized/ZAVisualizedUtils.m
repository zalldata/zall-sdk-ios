...
static NSInteger kZAVisualizedFindMaxPageLevel = 4;
typedef NSArray<UIView *>* (*ZASortedRNSubviewsMethod)(UIView *, SEL);

/// RCTView 响应交互类型
typedef NS_ENUM(NSInteger, ZARCTViewPointerEvents) {
    /// 0: 默认类型，优先使用子视图响应交互
    ZARCTViewPointerEventsUnspecified = 0,
    /// 1: 自身以及子视图都不响应交互，所以不阻塞下层 view 交互
    ZARCTViewPointerEventsNone,
    /// 2: 只让子视图响应交互，自身不可点击
    ZARCTViewPointerEventsBoxNone,
    /// 3: 只有自身接收事件，子视图不可交互
    ZARCTViewPointerEventsBoxOnly,
};

@implementation ZAVisualizedUtils

#pragma mark Covered
+ (BOOL)isCoveredForView:(UIView *)view {
    NSArray <UIView *> *allOtherViews = [self findAllPossibleCoverViews:view hierarchyCount:kZAVisualizedFindMaxPageLevel];
...
+ (BOOL)isCoveredOfRNView:(UIView *)view fromRNView:(UIView *)fromView {
     ...
     */
    ZARCTViewPointerEvents pointerEvents = [self pointEventsWithRCTView:fromView];
    // RCTView 重写 hitTest: 并返回 nil，不阻塞底下元素交互
    if (pointerEvents == ZARCTViewPointerEventsNone) {
        return NO;
    }
    // 遍历子视图判断是否存在坐标覆盖阻塞交互
    if (pointerEvents == ZARCTViewPointerEventsBoxNone) {
        // 寻找完全遮挡 view 的子视图
        for (UIView *subView in fromView.subviews) {
            BOOL enableInteraction = [ZAVisualizedUtils isVisibleForView:subView] && subView.userInteractionEnabled;
...
+ (NSArray *)findPossibleCoverAllBrotherViews:(UIView *)view {
    ...
    if ([self isKindOfRCTView:superView]) {
        /* RCTView 默认重写了 hitTest:
         ...
         详情参照：https://github.com/facebook/react-native/blob/master/React/Views/RCTView.m
         */
        ZARCTViewPointerEvents pointerEvents = [self pointEventsWithRCTView:superView];
        // RCTView 重写 hitTest: 并返回 nil，不阻塞底下元素交互
        if (pointerEvents != ZARCTViewPointerEventsNone && pointerEvents != ZARCTViewPointerEventsBoxOnly) {
...
+ (NSArray *)analysisWebElementWithWebView:(WKWebView <ZAVisualizedExtensionProperty> *)webView {
    ZAVisualizedWebPageInfo *webPageInfo = [[ZAVisualizedObjectSerializerManager sharedInstance] readWebPageInfoWithWebView:webView];
    ...
    NSMutableArray<ZAWebElementView *> *webElementArray = [NSMutableArray array];
    ...
    for (NSDictionary *pageData in webElementSources) {
        ...
        ZAWebElementView *webElement = [[ZAWebElementView alloc] initWithWebView:webView webElementInfo:pageData];
        if (webElement) {
    ...
    // 根据 level 升序排序
    [webElementArray sortUsingComparator:^NSComparisonResult(ZAWebElementView *obj1,ZAWebElementView *obj2) {
    ...
    // 构建子元素数组
    for (ZAWebElementView *webElement1 in [webElementArray copy]) {
        //当前元素是否嵌套子元素
        ...
        // 根据子元素 id 查找对应子元素
        for (ZAWebElementView *webElement2 in [webElementArray copy]) {
...
+ (NSArray<UIView *> *)sortedRNSubviewsWithView:(UIView *)view {
    ...
    ZASortedRNSubviewsMethod method = (ZASortedRNSubviewsMethod)[view methodForSelector:sortedRNSubviewsSel];
...
+ (BOOL)isInteractiveEnabledRNView:(UIView *)view {
    ...
    if (view.superview.zalldata_isDisableRNSubviewsInteractive) {
        view.zalldata_isDisableRNSubviewsInteractive = YES;
        return NO;
    }
    ...
    // 设置交互状态
    ZARCTViewPointerEvents pointerEvents = [self pointEventsWithRCTView:view];
    // None 和 BoxOnly 都禁用子视图交互
    BOOL isEventsNone = pointerEvents == ZARCTViewPointerEventsNone;
    BOOL isEventsBoxOnly = pointerEvents == ZARCTViewPointerEventsBoxOnly;
    view.zalldata_isDisableRNSubviewsInteractive = isEventsNone || isEventsBoxOnly;
    
    // EventsNone 和 EventsBoxNone 时，自身不可交互
    if (pointerEvents == ZARCTViewPointerEventsNone || pointerEvents == ZARCTViewPointerEventsBoxNone) {
        return NO;
    }
...
/// 获取当前 RCTView 的交互类型
+ (ZARCTViewPointerEvents)pointEventsWithRCTView:(UIView *)view {
    if (![self isKindOfRCTView:view]) {
        return ZARCTViewPointerEventsUnspecified;
    }
    
    ZARCTViewPointerEvents pointerEvents = ZARCTViewPointerEventsUnspecified;
    @try {
        pointerEvents = [[view valueForKey:@"pointerEvents"] integerValue];
    } @catch (NSException *exception) {
        ZALogWarn(@"%@ error: %@", self, exception);
    }
...
+ (UIImage *)screenshotWithView:(UIView *)view {
    ...
    @try {
        ...
        ZALogError(@"screenshot fail，error %@: %@", self, exception);
    }
...
+ (BOOL)isSupportCallJSWithWebView:(WKWebView *)webview {
    ...
    [userScripts enumerateObjectsUsingBlock:^(WKUserScript *_Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
        if ([obj.source containsString:kZAJSBridgeServerURL]) {
...
@implementation ZAVisualizedUtils (ViewPath)

+ (BOOL)isIgnoredViewPathForViewController:(UIViewController *)viewController {
    BOOL isEnableVisualized =  [[ZAVisualizedManager defaultManager] isVisualizeWithViewController:viewController];
...
+ (NSArray<NSString *> *)viewPathsForViewController:(UIViewController<ZAAutoTrackViewPathProperty> *)viewController {
    NSMutableArray *viewPaths = [NSMutableArray array];
    do {
        [viewPaths addObject:viewController.zalldata_heatMapPath];
        viewController = (UIViewController<ZAAutoTrackViewPathProperty> *)viewController.parentViewController;
    } while (viewController);

    UIViewController<ZAAutoTrackViewPathProperty> *vc = (UIViewController<ZAAutoTrackViewPathProperty> *)viewController.presentingViewController;
    if ([vc conformsToProtocol:@protocol(ZAAutoTrackViewPathProperty)]) {
        [viewPaths addObjectsFromArray:[self viewPathsForViewController:vc]];
    }
    return viewPaths;
}

+ (NSArray<NSString *> *)viewPathsForView:(UIView<ZAAutoTrackViewPathProperty> *)view {
    NSMutableArray *viewPathArray = [NSMutableArray array];
    do { // 遍历 view 层级 路径
        if (view.zalldata_heatMapPath) {
            [viewPathArray addObject:view.zalldata_heatMapPath];
        }
    } while ((view = (id)view.nextResponder) && [view isKindOfClass:UIView.class] && ![view isKindOfClass:UIWindow.class]);

    if ([view isKindOfClass:UIViewController.class] && [view conformsToProtocol:@protocol(ZAAutoTrackViewPathProperty)]) {
        // 遍历 controller 层 路径
        [viewPathArray addObjectsFromArray:[self viewPathsForViewController:(UIViewController<ZAAutoTrackViewPathProperty> *)view]];
    }
...
+ (NSString *)viewSimilarPathForView:(UIView *)view atViewController:(UIViewController *)viewController shouldSimilarPath:(BOOL)shouldSimilarPath {
    ...
    do {
        if (isContainSimilarPath || !shouldSimilarPath) { // 防止 cell 嵌套，被拼上多个 [-]
            if (view.zalldata_itemPath) {
                [viewPathArray addObject:view.zalldata_itemPath];
            }
        } else {
            NSString *currentSimilarPath = view.zalldata_similarPath;
    ...
    if ([view isKindOfClass:UIAlertController.class]) {
        UIViewController<ZAAutoTrackViewPathProperty> *viewController = (UIViewController<ZAAutoTrackViewPathProperty> *)view;
        [viewPathArray addObject:viewController.zalldata_itemPath];
    }
...
+ (NSString *)itemHeatMapPathForResponder:(UIResponder *)responder {
    ...
    NSInteger index = [ZAAutoTrackProperty itemIndexForResponder:responder];
