//
//  ZAApplicationStateSerializer.m
//  ZallDataSDK
//
...
@implementation ZAApplicationStateSerializer {
    ZAVisualizedAutoTrackObjectSerializer *_visualizedSerializer;
}
...
- (instancetype)initWithConfiguration:(ZAObjectSerializerConfig *)configuration
    ...
    self = [super init];
    if (self) {
        _visualizedSerializer = [[ZAVisualizedAutoTrackObjectSerializer alloc] initWithConfiguration:configuration objectIdentityProvider:objectIdentityProvider];
    }
...
- (void)screenshotImageForAllWindowWithCompletionHandler:(void (^)(UIImage *))completionHandler {
    ...
    for (UIWindow *window in allActiveWindows) {
        ...
        if ([ZAVisualizedUtils isVisibleForView:window] && !window.superview) {
    ...
    if (validWindows.count == 1) {
        UIImage *image = [ZAVisualizedUtils screenshotWithView:validWindows.firstObject];
        // 单张图片
    ...
    for (UIWindow *window in validWindows) {
        UIImage *image = [ZAVisualizedUtils screenshotWithView:window];
        if (image) {
