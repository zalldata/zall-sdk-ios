...
@implementation ZAViewIdentifier
...
- (instancetype)initWithView:(UIView *)view {
    self = [super init];
    if (self) {
        _elementPath = view.zalldata_elementPath;
        _screenName = view.zalldata_screenName;
        _elementContent = view.za_property_elementPosition;
        _elementPosition = view.za_property_elementPosition;
        _pageIndex = [ZAVisualizedUtils pageIndexWithView:view];
    }
    return self;
}
...
@implementation ZAVisualPropertiesEventConfig
...
@implementation ZAVisualPropertiesPropertyConfig
...
- (instancetype)initWithDictionary:(NSDictionary *)propertiesDic {
    ...
    if (self) {
        ...
        if ([type isEqualToString:@"NUMBER"]) {
            _type = ZAVisualPropertyTypeNumber;
        } else {
            _type = ZAVisualPropertyTypeString;
        }
...
/// h5 属性配置，匹配元素
- (BOOL)isEqualToWebViewIdentify:(ZAViewIdentifier *)object {
    if (!self.isH5) {
        return NO;
    }
...
@implementation ZAVisualPropertiesConfig
...
- (instancetype)initWithDictionary:(NSDictionary *)eventsDic {
    ...
    if (self) {
        ...
        if ([eventTypeString isEqualToString:@"appclick"]) {
            _eventType = ZAAutoTrackEventTypeAppClick;
        } else {
            _eventType = ZAAutoTrackEventTypeNone;
        }

        _eventName = dictionaryValueForKey(eventsDic, @"event_name");
        NSDictionary *eventDic = dictionaryValueForKey(eventsDic, @"event");
        _event = [[ZAVisualPropertiesEventConfig alloc] initWithDictionary:eventDic];

        NSArray<NSDictionary *> *propertiesArray = dictionaryValueForKey(eventsDic, @"properties");
        if (propertiesArray) {

            NSMutableArray<ZAVisualPropertiesPropertyConfig *> *properties = [NSMutableArray array];
            NSMutableArray <NSDictionary *>*webProperties = [NSMutableArray array];
            for (NSDictionary *dic in propertiesArray) {
                ZAVisualPropertiesPropertyConfig *config = [[ZAVisualPropertiesPropertyConfig alloc] initWithDictionary:dic];
...
@implementation ZAVisualPropertiesResponse
    ...
    if (self) {
        ...
        if (events.count > 0) {
            ...
            for (NSDictionary *eventDic in events) {
                ZAVisualPropertiesConfig *event = [[ZAVisualPropertiesConfig alloc] initWithDictionary:eventDic];
