...
NSString * const kZAService = @"com.zalldata.analytics.udid";
NSString * const kZAUdidAccount = @"com.zalldata.analytics.udid";

@implementation ZAKeyChainItemWrapper
+ (NSString *)saUdid {
    NSDictionary *result = [self fetchPasswordWithAccount:kZAUdidAccount service:kZAService];
    NSString *za_udid =  [result objectForKey:(__bridge id)kSecValueData];
    return za_udid;
}

+ (NSString *)saveUdid:(NSString *)udid {
    BOOL sucess = [self saveOrUpdatePassword:udid account:kZAUdidAccount service:kZAService];
...
+ (BOOL)saveOrUpdatePassword:(NSString *)password account:(NSString *)account service:(NSString *)service accessGroup:(NSString *)accessGroup {
    @try {
        ...
        if (status == errSecSuccess) {
            ...
            ZALogDebug(@"SecItemUpdate result = %d", result);
            ...
            ZALogDebug(@"SecItemAdd result = %d", status);
        }
