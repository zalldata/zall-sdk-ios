...
- (BOOL)open {
    ...
    if (sqlite3_open_v2([self.filePath UTF8String], &_database, SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE, NULL) != SQLITE_OK) {
        _database = NULL;
        ZALogError(@"Failed to open SQLite db");
        return NO;
    }
    ZALogDebug(@"Success to open SQLite db");
    self.isOpen = YES;
    return YES;
}
...
- (void)close {
    ...
    _isCreatedTable = NO;
    _isOpen = NO;
    ZALogDebug(@"%@ close database", self);
}
...
- (BOOL)createTable {
    ...
    if (sqlite3_exec(_database, sql.UTF8String, NULL, NULL, NULL) != SQLITE_OK) {
        ZALogError(@"Create %@ table fail.", kDatabaseTableName);
        self.isCreatedTable = NO;
        return NO;
    }
    if (![self createColumn:kDatabaseColumnStatus inTable:kDatabaseTableName]) {
        ZALogError(@"Alert table %@ add %@ fail.", kDatabaseTableName, kDatabaseColumnStatus);
        self.isCreatedTable = NO;
        return NO;
    }
    ...
    ZALogDebug(@"Create %@ table success, current count is %lu", kDatabaseTableName, self.count);
    return YES;
}
...
- (NSArray<ZAEventRecord *> *)selectRecords:(NSUInteger)recordSize {
    ...
    if (!stmt) {
        ZALogError(@"Failed to prepare statement, error:%s", sqlite3_errmsg(_database));
        return [contentArray copy];
    }
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        if (!jsonChar) {
            ZALogError(@"Failed to query column_text, error:%s", sqlite3_errmsg(_database));
        ...
        ZAEventRecord *record = [[ZAEventRecord alloc] initWithRecordID:recordID content:content];
...
- (BOOL)resetAllRecordsStatus {
    NSString *sql = [NSString stringWithFormat:@"UPDATE %@ SET %@ = %d WHERE %@ = (%d);", kDatabaseTableName, kDatabaseColumnStatus, ZAEventRecordStatusNone, kDatabaseColumnStatus, ZAEventRecordStatusFlush];
...
- (BOOL)execUpdateSQL:(NSString *)sql {
    ...
    if (sqlite3_prepare_v2(_database, sql.UTF8String, -1, &stmt, NULL) != SQLITE_OK) {
        ZALogError(@"Prepare update records query failure: %s", sqlite3_errmsg(_database));
        return NO;
    }
    if (sqlite3_step(stmt) != SQLITE_DONE) {
        ZALogError(@"Failed to update records from database, error: %s", sqlite3_errmsg(_database));
        return NO;
    }
...
- (BOOL)insertRecord:(ZAEventRecord *)record {
    if (![record isValid]) {
        ZALogError(@"%@ input parameter is invalid for addObjectToDatabase", self);
        return NO;
    }
    if (![self databaseCheck]) {
        return NO;
    ...
    if (insertStatement) {
        ...
        if (rc != SQLITE_DONE) {
            ZALogError(@"insert into dataCache table of sqlite fail, rc is %d", rc);
            return NO;
        }
        self.count++;
        ZALogDebug(@"insert into dataCache table of sqlite success, current count is %lu", self.count);
        return YES;
    } else {
        ZALogError(@"insert into dataCache table of sqlite error");
        return NO;
    }
}
...
- (BOOL)deleteRecords:(NSArray<NSString *> *)recordIDs {
    ...
    if (sqlite3_prepare_v2(_database, query.UTF8String, -1, &stmt, NULL) != SQLITE_OK) {
        ZALogError(@"Prepare delete records query failure: %s", sqlite3_errmsg(_database));
        return NO;
    }
    BOOL success = YES;
    if (sqlite3_step(stmt) != SQLITE_DONE) {
        ZALogError(@"Failed to delete record from database, error: %s", sqlite3_errmsg(_database));
        success = NO;
    }
    sqlite3_finalize(stmt);
    self.count = [self messagesCount];
...
- (BOOL)deleteAllRecords {
    ...
    if (sqlite3_exec(_database, sql.UTF8String, NULL, NULL, NULL) != SQLITE_OK) {
        ZALogError(@"Failed to delete all records");
        return NO;
    } else {
        ZALogDebug(@"Delete all records successfully");
    }
    self.count = 0;
    return YES;
}
...
- (BOOL)preCheckForInsertRecords:(NSUInteger)recordSize {
    NSInteger maxSize =ZAConfigOptions.sharedInstance.cacheMaxSize;
    if (recordSize > maxSize) {
        return NO;
    }
    while ((self.count + recordSize) >= maxSize) {
        ZALogWarn(@"AddObjectToDatabase touch MAX_MESSAGE_SIZE:%lu, try to delete some old events", maxSize);
        if (![self deleteFirstRecords:kRemoveFirstRecordsDefaultCount]) {
            ZALogError(@"AddObjectToDatabase touch MAX_MESSAGE_SIZE:%lu, try to delete some old events FAILED", maxSize);
            return NO;
        }
    }
    return YES;
...
- (sqlite3_stmt *)dbCacheStmt:(NSString *)sql {
    ...
    if (!stmt) {
        ...
        if (result != SQLITE_OK) {
            ZALogError(@"sqlite stmt prepare error (%d): %s", result, sqlite3_errmsg(_database));
            return NULL;
        }
...
- (NSArray<NSString *>*)columnsInTable:(NSString *)tableName {
    ...
    sqlite3_stmt *stmt;
    if (sqlite3_prepare_v2(_database, query.UTF8String, -1, &stmt, NULL) != SQLITE_OK) {
        ZALogError(@"Prepare PRAGMA table_info query failure: %s", sqlite3_errmsg(_database));
        return columns;
    }
...
- (BOOL)createColumn:(NSString *)columnName inTable:(NSString *)tableName {
    ...
    if (sqlite3_prepare_v2(_database, query.UTF8String, -1, &stmt, NULL) != SQLITE_OK) {
        ZALogError(@"Prepare create column query failure: %s", sqlite3_errmsg(_database));
        return NO;
    }
    if (sqlite3_step(stmt) != SQLITE_DONE) {
        ZALogError(@"Failed to create column, error: %s", sqlite3_errmsg(_database));
        return NO;
    }
    sqlite3_finalize(stmt);
    return YES;
...
- (NSUInteger)messagesCount {
    ...
    if (statement) {
        ...
        ZALogError(@"Failed to get count form dataCache");
    }
