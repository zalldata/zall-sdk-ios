...
@implementation ZATrackerAppTimer
...
- (NSString *)generateEventIdByEventName:(NSString *)eventName {
    ...
    if (![eventName hasSuffix:kZAEventIdSuffix]) {
        //生成计时事件的 eventId，结构为 {eventName}_{uuid}_ZATimer
        //uuid 字符串中 ‘-’ 是不合法字符，替换为 ‘_’
        NSString *uuid = [NSUUID.UUID.UUIDString stringByReplacingOccurrencesOfString:@"-" withString:@"_"];
        eventId = [NSString stringWithFormat:@"%@_%@%@", eventName, uuid, kZAEventIdSuffix];
    }
    return eventId;
}

- (void)trackTimerStart:(NSString *)eventId currentSysUpTime:(UInt64)currentSysUpTime {
    return [self trackTimerStart:eventId timeUnit:    ZATrackTimeUnitStateSeconds currentSysUpTime:currentSysUpTime];
}

- (void)trackTimerStart:(NSString *)eventId timeUnit:(ZATrackTimeUnitState)timeUnit currentSysUpTime:(UInt64)currentSysUpTime {
...
- (NSString *)eventNameFromEventId:(NSString *)eventId {
    if (![eventId hasSuffix:kZAEventIdSuffix]) {
        return eventId;
    }
...
- (NSNumber *)handleEventDuration:(NSString *)eventId mapping:(NSMutableDictionary *)mapping currentSystemUpTime:(UInt64)currentSystemUpTime {
    ...
    ZATrackTimeUnitState timeUnit = [[eventTimer valueForKey:@"timeUnit"] intValue];
...
- (void)handleAllEventsPause:(NSMutableDictionary *)mapping time:(UInt64)currentSystemUpTime {
    for (NSString *key in mapping.allKeys) {
        if (key != nil) {
            if ([key isEqualToString:kZAEventNameAppEnd]) {
                continue;
            }
        }
...
//计算事件时长
- (float)eventTimerDurationWithCurrentTime:(UInt64)currentSystemUpTime eventStart:(UInt64)startTime timeUnit:(ZATrackTimeUnitState)timeUnit {
    if (startTime <= 0) {
        return 0;
    }
    ...
    if (eventDuration > 0 && eventDuration < 24 * 60 * 60 * 1000) {
        switch (timeUnit) {
            case     ZATrackTimeUnitStateHours:
                eventDuration = eventDuration / 60.0;
            case     ZATrackTimeUnitStateMinutes:
                eventDuration = eventDuration / 60.0;
            case     ZATrackTimeUnitStateSeconds:
                eventDuration = eventDuration / 1000.0;
            case     ZATrackTimeUnitStateMilliseconds:
                break;
        }
    } else {
        eventDuration = 0;
