...
typedef NSURLSessionAuthChallengeDisposition (^ZAURLSessionDidReceiveAuthenticationChallengeBlock)(NSURLSession *session, NSURLAuthenticationChallenge *challenge, NSURLCredential * __autoreleasing *credential);
typedef NSURLSessionAuthChallengeDisposition (^ZAURLSessionTaskDidReceiveAuthenticationChallengeBlock)(NSURLSession *session, NSURLSessionTask *task, NSURLAuthenticationChallenge *challenge, NSURLCredential * __autoreleasing *credential);

@interface ZAHTTPSession () <NSURLSessionDelegate, NSURLSessionTaskDelegate>

@property (nonatomic, strong) NSURLSession *session;
@property (nonatomic, strong) NSOperationQueue *delegateQueue;

@property (nonatomic, copy) ZAURLSessionDidReceiveAuthenticationChallengeBlock sessionDidReceiveAuthenticationChallenge;
@property (nonatomic, copy) ZAURLSessionTaskDidReceiveAuthenticationChallengeBlock taskDidReceiveAuthenticationChallenge;

@end

@implementation ZAHTTPSession

+ (ZAHTTPSession *)sharedInstance {
    static dispatch_once_t onceToken;
    static ZAHTTPSession *session = nil;
    dispatch_once(&onceToken, ^{
        session = [[ZAHTTPSession alloc] init];
    });
    return session;
}
...
- (instancetype)init {
    self = [super init];
    if (self) {
        _securityPolicy = [ZASecurityPolicy defaultPolicy];

        _delegateQueue = [[NSOperationQueue alloc] init];
        _delegateQueue.name = [NSString stringWithFormat:@"cn.zalldata.ZAHTTPSession.%p", self];
