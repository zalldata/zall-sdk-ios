//
//  ZAAppExtensionDataManager.m
//  ZallDataSDK
//
//  Created by guo on 2018/1/18.
...
void *ZAAppExtensionQueueTag = &ZAAppExtensionQueueTag;

@interface ZAAppExtensionDataManager() {
}
@property (nonatomic, strong) dispatch_queue_t appExtensionQueue;
@end

@implementation ZAAppExtensionDataManager

+ (instancetype)sharedInstance {
    static ZAAppExtensionDataManager *manager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        manager = [[ZAAppExtensionDataManager alloc] init];
    });
    return manager;
}
...
- (instancetype)init {
    if (self = [super init]) {
        self.appExtensionQueue = dispatch_queue_create("com.zalldata.analytics.appExtensionQueue", DISPATCH_QUEUE_SERIAL);
        dispatch_queue_set_specific(self.appExtensionQueue, ZAAppExtensionQueueTag, &ZAAppExtensionQueueTag, NULL);
    }
    return self;
}
...
- (void)setGroupIdentifierArray:(NSArray *)groupIdentifierArray {
    ...
    if (dispatch_get_specific(ZAAppExtensionQueueTag)) {
        block();
    } else {
...
- (NSString *)filePathForApplicationGroupIdentifier:(NSString *)groupIdentifier {
    @try {
        ...
        dispatch_block_t block = ^() {
            NSURL *pathUrl = [[[NSFileManager defaultManager] containerURLForSecurityApplicationGroupIdentifier:groupIdentifier] URLByAppendingPathComponent:@"zall_event_data.plist"];
