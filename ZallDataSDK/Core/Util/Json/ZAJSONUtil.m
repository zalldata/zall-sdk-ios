...
+ (NSData *)dataWithJSONObject:(id)obj {
    ...
    @try {
        ...
        if (error) {
            ZALogError(@"%@ error encoding api data: %@", self, error);
        }
    }
    @catch (NSException *exception) {
        ZALogError(@"%@ exception encoding api data: %@", self, exception);
    }
    return data;
}
...
+ (NSString *)stringWithJSONObject:(id)obj {
    ...
    if (za_check_empty_data(jsonData)) {
        ZALogWarn(@"json data is invalid");
        return nil;
    }
...
+ (id)JSONSerializableObject:(id)obj {
    ...
    if ([newObj isKindOfClass:[NSDictionary class]]) {
        ...
        [(NSDictionary *)newObj enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, id  _Nonnull obj, BOOL * _Nonnull stop) {
            ...
            if (![key isKindOfClass:[NSString class]]) {
                ...
                ZALogWarn(@"property keys should be strings. but property: %@, type: %@, key: %@", newObj, [key class], key);
            }
    ...
    if ([newObj isKindOfClass:[NSDate class]]) {
        NSDateFormatter *dateFormatter = [ZAQuickUtil  zaDateFormatterFromString:nil];
    ...
    ZALogWarn(@"property values should be valid json types, but current value: %@, with invalid type: %@", newObj, [newObj class]);
    return [newObj description];
}

+ (id)JSONObjectWithData:(NSData *)data {
    if (za_check_empty_data(data)) {
        ZALogWarn(@"json data is invalid");
        return nil;
    }
...
+ (id)JSONObjectWithString:(NSString *)string options:(NSJSONReadingOptions)options {
    if (za_check_empty_string(string)) {
        ZALogWarn(@"string verify failure: %@", string);
        return nil;
    }
    NSData *data = [string dataUsingEncoding:NSUTF8StringEncoding];
    if (!data) {
        ZALogError(@"string dataUsingEncoding failure: %@",string);
        return nil;
    }
...
+ (id)JSONObjectWithData:(NSData *)data options:(NSJSONReadingOptions)options {
    ...
    @try {
        ...
        if (jsonError) {
            ZALogError(@"json serialization error: %@",jsonError);
        }
    } @catch (NSException *exception) {
